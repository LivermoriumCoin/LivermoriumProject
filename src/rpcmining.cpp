...
Value checkkernel(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
		throw JSONRPCError(-9, "livermorium is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(-10, "livermorium is downloading blocks...");
...
Value mine(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
		throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "livermorium is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "livermorium is downloading blocks...");
