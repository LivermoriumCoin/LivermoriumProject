...
int nAnonymizelivermoriumAmount = 1000;
...
#else
    const char* pszModule = "livermorium";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\livermorium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\livermorium
    // Mac: ~/Library/Application Support/livermorium
    // Unix: ~/.livermorium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "livermorium";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "livermorium.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "livermoriumd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong livermorium will not work properly.");
