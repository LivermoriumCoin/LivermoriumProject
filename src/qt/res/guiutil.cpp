...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("livermorium"))
        return false;
...
{
    // Convert livermorium:// to livermorium:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("livermorium://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "livermorium:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "livermorium.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for livermorium.lnk
...
{
    return GetAutostartDir() / "livermorium.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a livermorium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=livermorium\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the livermorium app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add livermorium app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("livermorium-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  livermorium-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("livermorium-Qt"));
