...
void Shutdown()
{
    ...
    RenameThread("livermorium-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: livermorium.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: livermoriumd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 7799)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 7798)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizelivermoriumamount=<n> " + _("Keep N livermorium anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. livermorium is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. livermorium is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("livermorium version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "livermorium server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of livermorium") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart livermorium to complete") << "\n";
    ...
    nAnonymizelivermoriumAmount = GetArg("-anonymizelivermoriumamount", 0);
    if(nAnonymizelivermoriumAmount > 999999) nAnonymizelivermoriumAmount = 999999;
    if(nAnonymizelivermoriumAmount < 2) nAnonymizelivermoriumAmount = 2;
    ...
    LogPrintf("Anonymize livermorium Amount %d\n", nAnonymizelivermoriumAmount);
